...
one.
  ...
  a) "A + B * C + D" parses as "A + ((B * C) + D)"
		...
		* is non-associative with precedence 2
		

  b) "A & B | C ^ D ^ E" parses as "(A & B) | ((C ^ D) ^ E)"
		| is non-associative with precedence 1
  ...
  c) "A % B @ C = D @ E % F" parses as "((A % B) @ C) = ((D @ E) % F)"
		= is non-associative with precedence 1
		@ is non-associative with precedence 2
		% is non-assocaitve with precedence 2
		
  d) "A + B * C = A * B + A * C" parses as "(A + (B * C)) = ((A * B) + (A * C))"
		= is non-associative with precedence 1
		+ is non-associative with precedence 2
		* is non-associaitve with precedence 3
...
1)
	"X" -> "E ^ (X)" -> "P ^ (X)" -> "A ^ (X)" -> "2 ^ (X)" -> "2 ^ (E ^ X)" ->
	"2 ^ (P ^ X)" -> "2 ^ (A ^ X)" -> "2 ^ (4 ^ X)" -> "2 ^ (4 ^ E)" ->
	"2 ^ (4 ^ P)" -> "2 ^ (4 ^ A)" -> "2 ^ (4 ^ 5)"
	
2)
	"X" -> "E ^ (X)" -> "E + P ^ (X)" -> "P + P ^ (X)" -> "A + P ^ (X)" ->
	"2 + P ^ (X)" -> "2 + A ^ (X)" -> "2 + 3 ^ (X)" -> "2 + 3 ^ (E ^ X)" ->
	"2 + 3 ^ (P ^ X)" -> "2 + 3 ^ (A ^ X)" -> "2 + 3 ^ (1 ^ X)" ->
	"2 + 3 ^ (1 ^ E)" -> "2 + 3 ^ (1 ^ P)" -> "2 + 3 ^ (1 ^ A)" ->
	"2 + 3 ^ ( 1 ^ 7)"
...
After switching the two, the compiler brings up an error and says that there
are left over tokens after parsing. What we can do is left-facter the grammer
to remove the CFG rules.
